...
类加载
	有哪些类加载器  
		系统类加载器   应用程序类加载器AppClassLoader
		平台类加载器   PlatformClassLoader
		启动类加载器   打印的是null
	
	类在什么时机会被加载 ?
		用到了一个类这个类就会被加载
		特殊情况:用到了类中的final变量类不会被加载的
	
	ClassLoader的两个方法
		getSystemClassLoader()    
			获取类加载器
		getResourceAsStream()  
        	加载某一个文件,这个文件必须放在src下
        	现在用不用他都可以,用IO流加载文件也行,但是后面web阶段两种写法是有区别的
...
反射
	...
	怎么暴力反射 ?
		所有的方法都是在中间加Declared
		getConstructor()   ---->    getDeclareadConstrutor()
		getMethod()        ---->    getDeclareadMethod();
		设置可以访问权限
		setAccessable(true);
		
	反射的作用  ?
	    不需要修改源代码,把要修改的部分放在配置文件中
	    反射是框架的灵魂!!!
```
...
xml
    ...
    语法:
    	元素
    		1.一个xml只能有一个根元素
    		2.元素分为开始标签和结束标签
    		3.如果元素中间没有内容,可以把元素定义成自闭合标签
    	
        属性
        	1.属性是键值对,键不能重复,值要用引号引起来,可以单引也可以是双引
        	2.属性写在开始标签 或 自闭合标签
        ...
        文档声明
        	<?xml version="1.0" encoding="UTF-8" ?>
        	文档声明可以不写,如果写就要写在第一行第一列
       
       XML的解析
       	    解析就是读取xml文件里面的数据
       	    DOM4J是解析的工具,DOM4J是第三方工具,用的时候需要导入jar包
       	    jar包如何导入   ?
       	    
       约束
       		约束是对xml做出限制的,规定能写什么不能写什么
